...
# build, install, configure an optional shell, and reload active Vuja
[group('build')]
[linux]
[macos]
install $shell="":
...
debug-install version="v1.0.0":
    ...
    OUT=$(BIN_DIR="$TMP/out1" VUJA_API_URL="http://localhost:${PORT}/happy" sh scripts/install.sh 2>&1) && STATUS=0 || STATUS=$?
    ...
    OUT=$(BIN_DIR="$TMP/out3" VUJA_API_URL="http://localhost:${PORT}/404" sh scripts/install.sh 2>&1) && STATUS=0 || STATUS=1
    ...
    OUT=$(BIN_DIR="$TMP/out4" VUJA_API_URL="http://localhost:${PORT}/ratelimit" sh scripts/install.sh 2>&1) && STATUS=0 || STATUS=1
