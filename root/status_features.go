...
func detectRepositoryStatus(directory string, includeStash bool) repositoryStatusSnapshot {
	...
	return detectRepositoryStatusAtRoot(root, includeStash)
}

func detectRepositoryStatusAtRoot(root string, includeStash bool) repositoryStatusSnapshot {
	gitDirectory := findGitDirectory(root)
