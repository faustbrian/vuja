...
func TestLoadZoxideDirectoriesCanBeDisabled(t *testing.T) {
	invoked := false
	entries := loadOptionalZoxideDirectories(t.Context(), false, func(context.Context) []scoring.DirectoryImport {
		invoked = true
		return []scoring.DirectoryImport{{Path: "/unexpected"}}
	})
	if invoked || len(entries) != 0 {
		t.Fatalf("expected disabled Zoxide import to skip execution, invoked=%v entries=%+v", invoked, entries)
	}
}
