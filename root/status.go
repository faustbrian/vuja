...
type statusEngine struct {
	...
	repositories     map[string]cachedRepositoryStatus
	run              statusCommand
	onUpdate         func(statusSnapshot)
	sample           func() (float64, float64)
	metrics          bool
...
const (
	statusVersionCacheLimit    = 256
	statusGitCacheLimit        = 128
	statusProjectCacheLimit    = 128
	statusRepositoryCacheLimit = 128
	defaultMetricHysteresis    = 2.0
)

var (
	statusVersionCacheMetrics    = cachemetrics.New("status-versions")
	statusGitCacheMetrics        = cachemetrics.New("status-git")
	statusProjectCacheMetrics    = cachemetrics.New("status-project")
	statusRepositoryCacheMetrics = cachemetrics.New("status-repository")
)
...
type cachedRepositoryStatus struct {
...
func newStatusEngine(options statusEngineOptions) *statusEngine {
	...
	metricHysteresis := options.MetricHysteresis
	...
	return &statusEngine{
		...
		metricHysteresis: metricHysteresis,
		skipVersions:     options.SkipVersions,
		allVersions:      options.AllVersions,
		versionAllow:     append([]string(nil), options.VersionAllow...),
		versionDeny:      append([]string(nil), options.VersionDeny...),
...
func (e *statusEngine) refreshMetrics() {
	if e.commandActive.Load() {
		return
	}
	cpu, memory := e.sample()
	e.mu.Lock()
	hysteresis := e.metricHysteresis
	if e.snapshot.HasCPU && e.snapshot.HasMemory &&
		math.Abs(e.snapshot.CPU-cpu) < hysteresis &&
		math.Abs(e.snapshot.Memory-memory) < hysteresis {
		e.mu.Unlock()
		return
	}
	e.snapshot.CPU = cpu
...
func (e *statusEngine) SetCommandActive(active bool) {
	e.commandActive.Store(active)
}
...
func (e *statusEngine) refresh(directory string) {
	...
	if !e.skipVersions {
		...
		if e.allVersions {
			for _, provider := range []string{"php", "composer", "python", "ruby", "elixir", "go", "node", "bun", "rust", "docker-compose", "docker"} {
				if _, exists := versionsToResolve[provider]; !exists {
					versionsToResolve[provider] = ""
				}
			}
		}
	}
	...
	for provider := range versionsToResolve {
		if slices.Contains(e.versionDeny, provider) || (len(e.versionAllow) > 0 && !slices.Contains(e.versionAllow, provider)) {
			delete(versionsToResolve, provider)
			delete(pinnedVersions, provider)
		}
	}
	previous := e.Snapshot()
	...
	shellStatus := e.shell
	e.mu.RUnlock()
	repository := e.repositoryStatus(directory)
	...
	probeGit := false
	if gitDir != "" {
		gitSignature = gitMetadataSignature(gitDir, repository.CommonDir)
		cached, cachedOK := e.cachedGitStatus(gitDir)
		if e.gitStash && cachedOK && cached.signature != gitSignature {
			repository.StashCount = countNonEmptyLines(filepath.Join(repository.CommonDir, "logs", "refs", "stash"))
			e.mu.Lock()
			e.repositories[repository.Root] = cachedRepositoryStatus{status: repository, refreshed: time.Now()}
			e.mu.Unlock()
		}
		if cachedOK {
		...
		probeGit = !cachedOK || cached.signature != gitSignature || time.Since(cached.refreshed) >= time.Second
	}
...
func (e *statusEngine) projectVersions(directory string) map[string]string {
	...
	if ok && cached.signature == signature {
		statusProjectCacheMetrics.Hit()
		e.mu.Lock()
		cached.refreshed = time.Now()
		e.projects[directory] = cached
		e.mu.Unlock()
		return cloneVersions(cached.versions)
	}
	statusProjectCacheMetrics.Miss()
	...
	pruneStatusCachesLocked(e)
	e.mu.Unlock()
	return versions
}

func (e *statusEngine) repositoryStatus(directory string) repositoryStatusSnapshot {
	root := findRepositoryRoot(directory)
	if root == "" {
		return repositoryStatusSnapshot{}
	}
	e.mu.RLock()
	cached, ok := e.repositories[root]
	e.mu.RUnlock()
	if ok && time.Since(cached.refreshed) < time.Second {
		statusRepositoryCacheMetrics.Hit()
		return cached.status
	}
	statusRepositoryCacheMetrics.Miss()
	status := detectRepositoryStatusAtRoot(root, e.gitStash)
	e.mu.Lock()
	e.repositories[root] = cachedRepositoryStatus{status: status, refreshed: time.Now()}
	pruneStatusCachesLocked(e)
	e.mu.Unlock()
	return status
}
...
func (e *statusEngine) cachedGitStatus(repository string) (cachedGitStatus, bool) {
	...
	e.mu.RUnlock()
	if ok {
		statusGitCacheMetrics.Hit()
	} else {
		statusGitCacheMetrics.Miss()
	}
	return value, ok
}
...
func gitMetadataSignature(gitDirectory string, commonDirectories ...string) string {
	var signature strings.Builder
	...
	if len(commonDirectories) > 0 && commonDirectories[0] != "" {
		path := filepath.Join(commonDirectories[0], "logs", "refs", "stash")
		if info, err := os.Stat(path); err == nil {
			signature.WriteString("stash:")
			signature.WriteString(strconv.FormatInt(info.ModTime().UnixNano(), 10))
			signature.WriteByte(':')
			signature.WriteString(strconv.FormatInt(info.Size(), 10))
			signature.WriteByte(';')
...
func (e *statusEngine) cachedVersion(key string) (string, bool) {
	...
	fresh := ok && time.Now().Before(value.expires)
	e.mu.RUnlock()
	if fresh {
		statusVersionCacheMetrics.Hit()
	} else {
		statusVersionCacheMetrics.Miss()
	}
...
func pruneStatusCachesLocked(engine *statusEngine) {
	for len(engine.versions) > statusVersionCacheLimit {
		oldestKey := ""
		var oldest time.Time
		for key, entry := range engine.versions {
			if oldestKey == "" || entry.expires.Before(oldest) {
				oldestKey, oldest = key, entry.expires
			}
		}
		delete(engine.versions, oldestKey)
		statusVersionCacheMetrics.Eviction()
	}
	for len(engine.git) > statusGitCacheLimit {
		oldestKey := ""
		var oldest time.Time
		for key, entry := range engine.git {
			if oldestKey == "" || entry.refreshed.Before(oldest) {
				oldestKey, oldest = key, entry.refreshed
			}
		}
		delete(engine.git, oldestKey)
		statusGitCacheMetrics.Eviction()
	}
	for len(engine.projects) > statusProjectCacheLimit {
		oldestKey := ""
		var oldest time.Time
		for key, entry := range engine.projects {
			if oldestKey == "" || entry.refreshed.Before(oldest) {
				oldestKey, oldest = key, entry.refreshed
			}
		}
		delete(engine.projects, oldestKey)
		statusProjectCacheMetrics.Eviction()
	}
	for len(engine.repositories) > statusRepositoryCacheLimit {
		oldestKey := ""
		var oldest time.Time
		for key, entry := range engine.repositories {
			if oldestKey == "" || entry.refreshed.Before(oldest) {
				oldestKey, oldest = key, entry.refreshed
			}
		}
		delete(engine.repositories, oldestKey)
		statusRepositoryCacheMetrics.Eviction()
	}
}
