...
func mergeFastResultsProfiled(query string, mode string) ([]spec.Suggestion, map[string]time.Duration) {
	...
	measure("fast-rank", func() {
		scored = scoring.Score(candidates, scoring.SignalSet{Query: strings.TrimSpace(query), Cwd: spec.GetCWD()})
	})
...
func scoreResultsProfiledContext(ctx context.Context, query string, mode string) ([]scoring.ScoredSuggestion, map[string]time.Duration) {
	...
	ctxArguments, cancelArguments := context.WithTimeout(ctx, 20*time.Millisecond)
	measure("arguments", func() {
		deduped = append(deduped, learnedArgumentSuggestions(ctxArguments, activeQuery, cwd, ws.Root, store)...)
	})
	cancelArguments()
	ctxRecovery, cancelRecovery := context.WithTimeout(ctx, 20*time.Millisecond)
	measure("recovery", func() {
	...
	measure("rank", func() { results = promoteAISuggestion(scoring.Score(deduped, signals)) })
	return results, timings
}
