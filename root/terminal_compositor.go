...
func (c *terminalCompositor) WritePTY(data []byte) {
	...
	c.stream.Consume(data, func(text []byte) {
		...
		if c.phase == terminalInput && !completingPromptRedraw {
...
func (c *terminalCompositor) finishCommandOutput(exitCode *int) {
	if c.commandCardOpen {
		...
		if c.inputBoxStatusEnabled() && exitCode != nil {
			now := time.Now
			if c.now != nil {
				now = c.now
			}
...
func (c *terminalCompositor) inputBoxSegmentsForSnapshot(
	...
	for _, name := range names {
		...
		case "contexts":
			...
			for _, context := range contextsForCommand(commandContext, snapshot.Contexts) {
				add(name, context, 85)
			}
		case "stale":
		...
		case "duration":
			if snapshot.ExitCode != nil {
				add(durationStatusColor(snapshot.Duration), formatStatusDuration(snapshot.Duration), 70)
			}
		case "exit":
			if snapshot.ExitCode == nil {
		...
		case "cpu":
			if snapshot.HasCPU || snapshot.CPU > 0 {
				add(loadStatusColor("cpu", snapshot.CPU), fmt.Sprintf("CPU %.0f%%", snapshot.CPU), 3)
			}
		case "memory":
			if snapshot.HasMemory || snapshot.Memory > 0 {
				add(loadStatusColor("memory", snapshot.Memory), fmt.Sprintf("RAM %.0f%%", snapshot.Memory), 2)
			}
		}
	}
	return segments
