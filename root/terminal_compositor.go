...
func (c *terminalCompositor) writePTYChunk(data []byte) {
	...
	c.stream.Consume(data, func(text []byte) {
		...
		if c.phase == terminalOutput && c.rawCommandPending {
...
func (c *terminalCompositor) Resize(width, height int) {
	...
	if c.rawCommandPending {
		c.renderPendingRawCommand()
	} else if c.phase == terminalInput {
		c.renderPinned()
	}
}
...
func (c *terminalCompositor) renderPendingRawCommand() {
	var frame strings.Builder
	frame.WriteString(terminalSyncStart)
	oldTop, oldRows := c.currentTransientUIRegion()
	c.clearAndDisableTransientUI(&frame)
	...
	baseStart := frame.Len()
	frame.WriteString("\x1b[?25l\x1b[r")
	originalTop := clamp(c.height-max(c.surfaceRows, 1), 0, c.height-1)
	for row := originalTop; row < c.height; row++ {
		fmt.Fprintf(&frame, "\x1b[%d;1H\x1b[2K", row+1)
	}

	contentRows := max(c.surfaceContentRows, 1)
	decorated := c.inputBoxDecorationRows() > 0
	cardLines := make([]string, 0, contentRows+c.inputBoxDecorationRows())
	if decorated {
		if c.inputBoxTitleEnabled() {
			cardLines = append(cardLines, c.inputBoxTitleLine())
		}
		cardLines = append(cardLines, c.completedInputBoxPaddingLine())
	}
	for offset := 0; offset < contentRows; offset++ {
		line := ""
		if offset < len(c.surfaceContentLines) {
			line = c.surfaceContentLines[offset]
		}
		if offset < len(c.surfaceContentCells) {
			line = c.completedSurfaceContentLine(c.surfaceContentCells[offset])
		}
		if decorated {
			line = c.completedInputBoxContent(line)
		}
	...
	if decorated {
		...
		if c.inputBoxStatusEnabled() {
			cardLines = append(cardLines, c.inputBoxStatusLines()...)
		}
	}
	cardRows := max(len(cardLines), 1)
	targetTop := clamp(c.height-cardRows, 0, c.height-1)
	for offset, line := range cardLines {
		fmt.Fprintf(&frame, "\x1b[%d;1H%s\x1b[0m", targetTop+offset+1, line)
	}
	baseEnd := frame.Len()
	frame.WriteString(terminalSyncEnd)
	contents := frame.String()
	_, _ = io.WriteString(c.out, contents)
	c.recordBackdrop([]byte(contents[baseStart:baseEnd]))
	c.surfaceRows = cardRows
	c.renderedTop = targetTop
	c.renderedLines = nil
	c.transientGeometryDirty = true
	c.rawCommandPending = true
}
