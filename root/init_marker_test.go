...
emit fish_postexec`,
	...
	for _, test := range tests {
		t.Run(test.shell, func(t *testing.T) {
			...
			if err != nil {
				t.Fatalf("%s integration failed: %v: %s", test.shell, err, output)
			}
...
`, integrationPath)
	...
	output, err := command.CombinedOutput()
	if err != nil {
		t.Fatalf("fish history-policy integration failed: %v: %s", err, output)
	}
	if !strings.Contains(string(output), commandStartIgnoreMessage+"\x00") {
...
`, integrationPath)
	...
	output, err := command.CombinedOutput()
	if err != nil {
		t.Fatalf("fish late history-policy integration failed: %v: %s", err, output)
	}
	if !strings.Contains(string(output), commandStartIgnoreMessage+"\x00") {
...
`, integrationPath)
	...
	output, err := command.CombinedOutput()
	if err != nil {
		t.Fatalf("fish repeated history-policy integration failed: %v: %s", err, output)
	}
