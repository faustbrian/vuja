...
func TestDirectoryPolicyRankExplainsIndexedOrder(t *testing.T) {
	if got := directoryPolicyRank(spec.Suggestion{Source: "directory-index", Priority: 100}); got != 1 {
		t.Fatalf("expected first policy rank, got %d", got)
	}
	if got := directoryPolicyRank(spec.Suggestion{Source: "filesystem", Priority: 100}); got != 0 {
		t.Fatalf("expected non-indexed suggestion to have no policy rank, got %d", got)
	}
}
