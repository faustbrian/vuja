...
func runWatchdog() {
	...
	// retain child stderr for crash reporting while filtering watchdog-only noise
	var stderrBuf bytes.Buffer
	var stderrDisplay watchdogStderrDisplay
			...
			if !suppress {
				...
				if triggerIdx != -1 {
					...
					if triggerIdx > printedLen {
						_, _ = origStderr.Write(stderrDisplay.Consume(currentContent[printedLen:triggerIdx], false))
					}
					stderrDisplay.Reset()
				} else {
					_, _ = origStderr.Write(stderrDisplay.Consume(tempBuf[:n], false))
				}
			}
		}
		if errRead != nil {
	...
	if !suppress {
		_, _ = origStderr.Write(stderrDisplay.Consume(nil, true))
	}
