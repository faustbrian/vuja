...
func TestGeneratedShellHooksAreSyntacticallyValid(t *testing.T) {
	for _, shellName := range []string{"zsh", "bash", "fish"} {
		t.Run(shellName, func(t *testing.T) {
			...
			hookPath := filepath.Join(t.TempDir(), "init."+shellName)
			if err := os.WriteFile(hookPath, []byte(shellInitScript(shellName, "/tmp/vuja")), 0600); err != nil {
				t.Fatal(err)
			}
			command := exec.CommandContext(t.Context(), shellPath, "-n", hookPath)
