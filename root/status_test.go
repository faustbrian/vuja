...
func TestGitCacheAvoidsPostCommandProbeWhenMetadataIsUnchanged(t *testing.T) {
	dir := t.TempDir()
	if err := os.Mkdir(filepath.Join(dir, ".git"), 0o755); err != nil {
		t.Fatal(err)
	}
	...
	if got := engine.Snapshot().Git.Branch; got != "main" {
		t.Fatalf("expected cached branch, got %q", got)
	}
	if got := calls.Load(); got != 1 {
		t.Fatalf("expected unchanged metadata to skip post-command Git probe, got %d", got)
	}
}
...
func TestCommandCompletionDebouncesGitRefreshes(t *testing.T) {
	...
	if got := calls.Load(); got != 1 {
		t.Fatalf("expected coalesced post-command refresh to reuse unchanged Git metadata, got %d", got)
	}
}
...
func TestStatusCachesAreBounded(t *testing.T) {
	engine := newStatusEngine(statusEngineOptions{})
	defer engine.Close()
	engine.mu.Lock()
	for index := 0; index < statusVersionCacheLimit+20; index++ {
		key := strconv.Itoa(index)
		engine.versions[key] = cachedStatusVersion{expires: time.Unix(int64(index+1), 0)}
	}
	for index := 0; index < statusGitCacheLimit+20; index++ {
		key := strconv.Itoa(index)
		engine.git[key] = cachedGitStatus{refreshed: time.Unix(int64(index+1), 0)}
	}
	for index := 0; index < statusProjectCacheLimit+20; index++ {
		key := strconv.Itoa(index)
		engine.projects[key] = cachedProjectVersions{refreshed: time.Unix(int64(index+1), 0)}
	}
	for index := 0; index < statusRepositoryCacheLimit+20; index++ {
		key := strconv.Itoa(index)
		engine.repositories[key] = cachedRepositoryStatus{refreshed: time.Unix(int64(index+1), 0)}
	}
	pruneStatusCachesLocked(engine)
	versions, git, projects, repositories := len(engine.versions), len(engine.git), len(engine.projects), len(engine.repositories)
	engine.mu.Unlock()
	if versions > statusVersionCacheLimit || git > statusGitCacheLimit || projects > statusProjectCacheLimit || repositories > statusRepositoryCacheLimit {
		t.Fatalf("expected bounded status caches, got versions=%d git=%d projects=%d repositories=%d", versions, git, projects, repositories)
	}
}

func TestStatusMetricsIgnoreSmallSamplingJitter(t *testing.T) {
	updates := 0
	engine := newStatusEngine(statusEngineOptions{
		Metrics:  true,
		Sample:   func() (float64, float64) { return 51.4, 40.9 },
		OnUpdate: func(statusSnapshot) { updates++ },
	})
	defer engine.Close()
	engine.mu.Lock()
	engine.snapshot.CPU, engine.snapshot.Memory = 50, 40
	engine.snapshot.HasCPU, engine.snapshot.HasMemory = true, true
	engine.mu.Unlock()
	engine.refreshMetrics()
	if updates != 0 {
		t.Fatalf("expected sub-threshold metric jitter not to publish, got %d updates", updates)
	}
}

func TestStatusMetricsPauseWhileCommandProducesOutput(t *testing.T) {
	var samples atomic.Int32
	...
	engine.refreshMetrics()
	if samples.Load() != 0 {
		t.Fatal("expected command output to pause metric sampling")
	}
	engine.SetCommandActive(false)
	engine.refreshMetrics()
	if samples.Load() != 1 {
		t.Fatal("expected metric sampling to resume after command output")
	}
}
...
func TestStatusEngineAlwaysVersionsHonorsProviderAllowListBeforeProbing(t *testing.T) {
	directory := t.TempDir()
	var mu sync.Mutex
	var commands []string
	engine := newStatusEngine(statusEngineOptions{
		AllVersions:  true,
		VersionAllow: []string{"go"},
		VersionDeny:  []string{"rust"},
		Run: func(_ context.Context, _ string, name string, _ ...string) ([]byte, error) {
			mu.Lock()
			commands = append(commands, name)
			mu.Unlock()
			if name == "go" {
				return []byte("go version go1.26.0 darwin/arm64\n"), nil
			}
			return nil, errors.New("unexpected provider")
		},
	})
	...
	engine.Refresh(directory)
	deadline := time.Now().Add(time.Second)
	for engine.Snapshot().Versions["go"] == "" && time.Now().Before(deadline) {
		time.Sleep(time.Millisecond)
	}
	if got := engine.Snapshot().Versions["go"]; got != "1.26.0" {
		t.Fatalf("expected Go version from the allowed provider, got %q", got)
	}
	mu.Lock()
	defer mu.Unlock()
	if len(commands) != 1 || commands[0] != "go" {
		t.Fatalf("expected only the allowed provider to run, got %v", commands)
	}
}
