...
func TestBottomCompositorRendersPaddedBorderlessInputAndStatusSurfaces(t *testing.T) {
	...
	if got := screenLine(screen, 6); !strings.HasPrefix(got, "  › git status") || strings.ContainsAny(got, "│╭╮") {
		t.Fatalf("expected padded borderless input surface, got %q", got)
	}
	if got := screenLine(screen, 7); strings.TrimSpace(got) != "" {
		t.Fatalf("expected bottom input padding, got %q", got)
	}
	if got := screenLine(screen, 8); !strings.HasPrefix(got, "  /tmp/project") || strings.ContainsAny(got, "│├┤╰╯") {
		t.Fatalf("expected borderless status row, got %q", got)
	}
	if got := screen.CursorPosition(); got.X != 14 || got.Y != 6 {
...
func TestBottomCompositorOutputScrollbackEmitsOnlyRawCommandOutput(t *testing.T) {
	var output bytes.Buffer
	const marker = "test-session"
	compositor := newTerminalCompositor(&output, "bottom", marker, 48, 16)
	compositor.SetInputBoxTheme(testInputBoxTheme())
	...
	if rendered := strings.Join(transition, "\n"); strings.Contains(rendered, "/tmp/project") || strings.Contains(rendered, "λ printf plain-output") {
		t.Fatalf("expected output-only scrollback to discard the active prompt snapshot, got %q", transition)
	}

	output.Reset()
	compositor.WritePTY([]byte("plain-output\r\n"))
	compositor.WritePTY(terminalMarkerBytes(marker, "command-end:0"))
	if got := output.String(); got != "plain-output\r\n" {
		t.Fatalf("expected output-only scrollback to preserve raw command output exactly, got %q", got)
	}
