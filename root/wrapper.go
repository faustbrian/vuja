...
func runWrapper() {
	...
	go func() {
		...
		for scanner.Scan() {
			...
			if query == "VUJA_CMD_STOP" || strings.HasPrefix(query, "VUJA_CMD_STOP:") {
				...
				bufferMu.Unlock()
				if cmdToRecord != "" && !policy.IsSensitive(cmdToRecord) {
					cwd := spec.GetCWD()
	...
	renderHistorySearch = func() {
		...
		writeStdout([]byte(output.String()))
	}
		...
		if n > 0 {
			...
			for i := 0; i < n; i++ {
				...
				if b == '\033' {
					...
					if sequenceLength := nextTokenAcceptSequenceLength(inputSlice, i); sequenceLength > 0 && !historySearch.Active() {
						bufferMu.Lock()
						...
						bufferMu.Unlock()
						if consumeNextTokenAcceptance(sequenceLength, ghostText) {
							intercepted = true
				...
				if b == 0x12 { // ctrl+r: open history search or cycle its scope
					intercepted = true
					...
					renderHistorySearch()
					continue
				} else if historySearch.Active() && b == 0x13 { // ctrl+s: successful commands only
					intercepted = true
					historySearch.ToggleSuccessfulOnly()
					renderHistorySearch()
					continue
				} else if historySearch.Active() && (b == 0x7f || b == 0x08) {
					intercepted = true
