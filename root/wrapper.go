			...
			for i := 0; i < n; i++ {
				...
				if b == '\033' {
					...
					if i+2 < n && (inputSlice[i+1] == '[' || inputSlice[i+1] == 'O') {
						...
						bufferMu.Unlock()
						if emptyPrompt && inputSlice[i+2] == 'A' && !historySearch.Active() {
							intercepted = true
							cancelSuggestionWork()
							suggestionResults.invalidate()
							presentOverlay(overlay.ClearAndDisable)
							...
							i += 2
							continue
						} else if overlay.IsVisible() && (inputSlice[i+2] == 'A' || inputSlice[i+2] == 'B') {
							intercepted = true
							userNavigated.Store(true)
							...
							if historyNav.Mirrored() {
								...
								if moved {
									...
									cursorOffset = 0
									bufferMu.Unlock()
									_, _ = ptmx.Write(historyPromptReplacement(selectedCmd))
