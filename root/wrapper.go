...
func runWrapper() {
	...
	c.Env = slices.DeleteFunc(c.Env, func(value string) bool {
		return strings.HasPrefix(value, "VUJA_MARKER=") ||
			...
			strings.HasPrefix(value, "VUJA_PROMPT_TEXT=")
	})
	if terminalMarkerID != "" {
		c.Env = append(c.Env, "VUJA_MARKER="+terminalMarkerID)
		c.Env = append(c.Env,
			"VUJA_MANAGED_PROMPT="+terminalMarkerID,
	...
	overlay.SetTerminalSize(initialWidth, initialHeight)
		...
		for {
			...
			if err != nil {
				if terminalPTYClosed(err) {
					display.WritePTY(codexResumeLinks.Flush())
					display.Close()
					restoreTerminal()
					os.Exit(0)
				}
			...
			display.WritePTY(codexResumeLinks.Transform(buf[:n]))
	...
	renderMenuNow = func() {
		...
		go func() {
			...
			for phase, duration := range timings {
				suggestionLatency.record("source-"+phase, duration)
				if strings.HasSuffix(phase, "-timeout") ||
					((phase == "arguments" || phase == "recovery") && duration >= 20*time.Millisecond) ||
					(phase == "signals" && duration >= suggestionSignalBudget) {
	...
	renderHistorySearch = func() {
		uiPresenter.Update(func(compose func(func() []byte)) {
			...
			compose(func() []byte {
				...
				historyNav.Select(overlay.GetCurrentCmd())
				output.WriteString(overlay.Render())
				return []byte(output.String())
			})
		})
	...
	for {
		...
		if n > 0 {
			...
			for i := 0; i < n; i++ {
				...
				navigationInputs, navigationLength := inputBindings.navigationRun(inputSlice, i)
				...
				if navigationLength > 0 {
					navigationActions := overlayNavigationActions(navigationInputs)
					bufferMu.Lock()
					emptyPrompt := naiveBuffer == ""
					bufferMu.Unlock()

					if emptyPrompt && len(navigationActions) > 0 && navigationActions[0] == integration.NavigationUp && !historySearch.Active() {
						intercepted = true
						cancelSuggestionWork()
						suggestionResults.invalidate()
						presentOverlay(overlay.ClearAndDisable)
						if !integration.HistoryLoaded() {
							_ = integration.EnsureHistoryLoaded(context.Background())
						}
						results := integration.CurrentRecentRichHistory(time.Now(), config.Get().UI.MaxSuggestions)
						if len(results) > 0 {
							selected := ""
							uiPresenter.Update(func(_ func(func() []byte)) {
								overlay.SetRecentHistory(results)
								if len(navigationActions) > 1 {
									overlay.NavigateMany(navigationActions[1:])
								}
								selected = overlay.GetCurrentCmd()
							})
							bufferMu.Lock()
							historyNav.BeginMirrored(naiveBuffer)
							...
							presentOverlay(overlay.Render)
						}
						i += navigationLength - 1
						continue
					}
					...
					if overlay.IsVisible() {
						intercepted = true
						userNavigated.Store(true)
						if historyNav.Mirrored() {
							var navigation integration.NavigationResult
							uiPresenter.Update(func(_ func(func() []byte)) {
								navigation = overlay.NavigateMany(navigationActions)
							})
							if navigation.Moved {
								historyNav.Select(navigation.SelectedCmd)
								bufferMu.Lock()
								naiveBuffer = navigation.SelectedCmd
								cursorOffset = 0
								bufferMu.Unlock()
								_, _ = ptmx.Write(historyPromptReplacement(navigation.SelectedCmd))
							}
							if navigation.Moved || navigation.Wrapped {
								presentOverlay(overlay.Render)
							}
						} else {
							bufferMu.Lock()
							...
							if navigation.Moved && historyNav.Active() {
								historyNav.Select(navigation.SelectedCmd)
							}
						}
						i += navigationLength - 1
				...
				if b == '\033' {
					...
					if i+2 < n && (inputSlice[i+1] == '[' || inputSlice[i+1] == 'O') {
						if !disableGhostText.Load() && inputSlice[i+2] == 'C' { // right arrow
							bufferMu.Lock()
							atEnd := (cursorOffset == 0)
				...
				if historySearch.Active() && (b == 0x7f || b == 0x08) {
					...
					recentHistoryWasActive := historyNav.Mirrored()
					historySearch.Close()
					...
					if overlay.IsVisible() && (overlay.GetUserNavigated() || searchWasActive) {
						...
						if selected != "" {
							...
							currentMode := activeMode
							activeModeMu.RUnlock()
							if currentMode == "spec" && !searchWasActive && !recentHistoryWasActive {
...
func terminalPTYClosed(err error) bool {
