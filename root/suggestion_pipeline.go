...
func (p *suggestionPipeline) storeLocked(request suggestionRequest, results []spec.Suggestion) {
	if request.mode == p.last.request.mode && request.cwd == p.last.request.cwd &&
		strings.HasPrefix(strings.ToLower(request.query), strings.ToLower(p.last.request.query)) {
		results = stabilizeSuggestionOrder(p.last.results, results)
	}
...
func stabilizeSuggestionOrder(previous, next []spec.Suggestion) []spec.Suggestion {
	result := clonePipelineSuggestions(next)
	if len(previous) == 0 || len(result) < 2 || previous[0].Cmd == result[0].Cmd {
		return result
	}
	priorIndex := -1
	for index := range result {
		if result[index].Cmd == previous[0].Cmd {
			priorIndex = index
			break
		}
	}
	...
	newStrength := result[0].Priority + result[0].Confidence
	oldStrength := result[priorIndex].Priority + result[priorIndex].Confidence
	if newStrength >= oldStrength+10 {
		return result
	}
	prior := result[priorIndex]
	copy(result[1:priorIndex+1], result[:priorIndex])
	result[0] = prior
	return result
}
...
func sameSuggestionResults(left, right []spec.Suggestion) bool {
	...
	for index := range left {
		if left[index].Cmd != right[index].Cmd || left[index].Source != right[index].Source ||
			left[index].Desc != right[index].Desc || left[index].Confidence != right[index].Confidence ||
