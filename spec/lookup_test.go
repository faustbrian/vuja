...
func TestLookupPreservesFilesystemSuggestionSource(t *testing.T) {
	originalDir, err := os.Getwd()
	if err != nil {
		t.Fatal(err)
	}
	tempDir := t.TempDir()
	if err := os.Mkdir(filepath.Join(tempDir, "scalar-docs"), 0755); err != nil {
		t.Fatal(err)
	}
	...
	results := Lookup("cd scalar-")
	if len(results) != 1 {
		t.Fatalf("expected one matching directory, got %+v", results)
	}
	if results[0].Cmd != "cd scalar-docs/" {
		t.Fatalf("expected scalar-docs completion, got %q", results[0].Cmd)
	}
	if results[0].Source != "filesystem" {
		t.Fatalf("expected filesystem source, got %q", results[0].Source)
	}
}

func TestLookupFastUsesStaticSpecsWithoutRunningGenerators(t *testing.T) {
	originalRegistry := Registry
	...
	Register(&Spec{
		...
		Subcommands: []Subcommand{{
			Name: "staging",
			Generator: func(context.Context, []string, string, string) []Suggestion {
				generatorCalled = true
				return []Suggestion{{Cmd: "remote"}}
			},
		}},
	})

	results := LookupFast("deploy sta")
	if generatorCalled {
		t.Fatal("fast lookup must not run command generators")
	}
	if !hasSuggestion(results, "deploy staging") {
		t.Fatalf("expected static subcommand from fast lookup, got %#v", results)
	}
}
