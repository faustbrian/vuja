...
func SetShellCWD(cwd string) {
	shellCWDMu.Lock()
	defer shellCWDMu.Unlock()
	if cwd == "" {
		shellCWD = ""
		return
	}
	if filepath.IsAbs(cwd) {
		shellCWD = filepath.Clean(cwd)
	}
}
...
func FileGenerator(filters ...string) GeneratorFunc {
	...
	return ImmediateGenerator(func(ctx context.Context, tokens []string, prefix string, partial string) []Suggestion {
		base := GetCWD()
		dir := base
		filePrefix := partial
