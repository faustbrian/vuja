...
func Init(logFilePath string, debug bool) {
	...
	lvlEnv := strings.ToLower(os.Getenv("VUJA_LOG_LEVEL"))
	switch lvlEnv {
	case "debug":
	...
	if err := config.EnsurePrivateDir(filepath.Dir(logFilePath)); err != nil {
		return
	}
	if err := config.RestrictPrivateFiles(logFilePath, logFilePath+".old"); err != nil {
		return
	}
	if info, err := os.Stat(logFilePath); err == nil && info.Size() > 5*1024*1024 {
		if err := os.Rename(logFilePath, logFilePath+".old"); err != nil {
			return
		}
		if err := config.RestrictPrivateFiles(logFilePath + ".old"); err != nil {
			return
		}
	}

	f, err := config.OpenPrivateFile(logFilePath, os.O_CREATE|os.O_WRONLY|os.O_APPEND)
	if err == nil {
		logFile = f
	}
}
