...
`, event.EventKey, event.Command, strings.TrimSpace(event.Cwd), event.StartedAt,
...
`)
	...
	for _, event := range events {
		...
		if _, err := stmt.ExecContext(ctx, event.EventKey, event.Command, strings.TrimSpace(event.Cwd),
			event.StartedAt, event.Duration.Nanoseconds(), exitCode, event.Source,
...
`, limit)
	...
	for rows.Next() {
		...
		event.StartedAt, _ = parseTimestamp(startedAtRaw)
