...
func ScoreWithConfig(suggestions []spec.Suggestion, signals SignalSet, config ScoreConfig) []ScoredSuggestion {
	...
	for i, s := range suggestions {
		...
		directoryAffinity := directoryAffinityFor(s.Cmd, localMap, globalMap)
...
func directoryAffinityFor(command string, local, global map[string]float64) int {
	localScore := local[command]
	if localScore <= 0 {
		return 0
	}
	globalScore := global[command]
	if globalScore <= 0 {
		return 100
	}
	return min(int(math.Round(localScore/globalScore*100)), 100)
}
