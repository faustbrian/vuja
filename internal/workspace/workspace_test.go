...
func TestGitStatusCacheIsBounded(t *testing.T) {
	gitStatusCache.Lock()
	original := gitStatusCache.entries
	gitStatusCache.entries = make(map[string]gitStatus)
	for index := 0; index < gitStatusCacheLimit+20; index++ {
		gitStatusCache.entries[fmt.Sprintf("/repo/%d", index)] = gitStatus{updatedAt: time.Unix(int64(index+1), 0)}
	}
	pruneGitStatusCacheLocked("/repo/current")
	size := len(gitStatusCache.entries)
	gitStatusCache.entries = original
	gitStatusCache.Unlock()
	if size > gitStatusCacheLimit {
		t.Fatalf("expected at most %d cached repositories, got %d", gitStatusCacheLimit, size)
	}
}
...
func TestDetect_InheritsRepositoryRootContextFromNestedDirectory(t *testing.T) {
	root := t.TempDir()
	child := filepath.Join(root, "internal", "service")
	...
	info := Detect(child)
	if info.Root != root {
		t.Fatalf("expected repository root %q, got %q", root, info.Root)
	}
	if !info.HasGit || !info.HasNodeProject {
		t.Fatalf("expected inherited Git and Node context, got %+v", info)
	}
}
