...
the defaults; Vuja-specific actions can be remapped under `keybindings`.
...
prompt-position = "classic" # "classic" or "bottom"
nerd-fonts = true
ghost-text = true
max-suggestions = 100
max-height = 15
...
separator = " · "
scrollback = "output"
path-color-mode = "hierarchy" # "hierarchy" or "single"
path-max-segments = 6         # 0 disables segment-based shortening
history-spacing = 1           # blank rows between completed executions
...
The hyphenated key follows Vuja's existing TOML naming convention. The default
is `classic`, so existing configurations retain normal shell positioning.
...
bounded second row when useful. Narrow terminals remove metrics, successful
exit state, versions, and low-risk detail before directory, branch, operations,
conflicts, mismatches, stale state, risky session context, or failures.
...
Duration is fast below `500ms`, average below `2s`, and slow from `2s`. CPU and
RAM are low below `50%`, average below `75%`, high below `90%`, and critical
from `90%`. Before the first command, exit state is neutral and duration is
hidden. Exit code zero uses the success color and non-zero codes use the failure
color.
...
Keybindings accept `tab`, `shift+tab`, `ctrl+a` through `ctrl+z`,
`alt+right`, `ctrl+right`, and `meta+right`. Set a single binding to `none`
or `accept-token` to `[]` to disable it. Vuja rejects duplicate bindings and
keeps Ctrl+A, Ctrl+C, Ctrl+E, Ctrl+H, Ctrl+J, Ctrl+L, Ctrl+M, Ctrl+U, Ctrl+W,
and Escape reserved for line editing.
...
records native Vuja executions without doing database work while you type.
...
status-layout, Git-cache, repository-relevance, version-cache, system-metric,
and full prompt-redraw benchmarks five times with allocation reporting. Compare
...
`history`, `learned`, `visited`, `directory`, or `command`.
