...
`vuja setup [shell]` installs the current executable, writes a static shell hook
under `~/.local/share/vuja`, and sources it first in the shell configuration.
The static hook avoids starting a separate `vuja init` process whenever a
terminal opens. Vuja launches the interactive shell itself; placing the
bootstrap later would make earlier initialization run in both shells.
...
<kbd>Ctrl</kbd> + <kbd>R</kbd> searches anywhere in the complete command and
...
`just bench` runs deterministic 200-candidate ranking and 500-entry
filesystem-completion fixtures five times with allocation reporting. Compare
the raw `ns/op`, `B/op`, and `allocs/op` results between commits; the repository
does not enforce a wall-clock CI threshold because shared runners are noisy.
