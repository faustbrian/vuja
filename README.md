...
| <kbd>↑</kbd> | Move up with wraparound; on an empty prompt, open recent executions with the newest selected at the bottom |
| <kbd>↓</kbd> | Move down with wraparound |
| <kbd>Page Up</kbd> / <kbd>Page Down</kbd> | Move by one visible suggestion page with wraparound |
...
Keybindings accept `tab`, `shift+tab`, `up`, `down`, `page-up`, `page-down`,
`ctrl+a` through `ctrl+z`, `home`, `end`, `alt+right`, `ctrl+right`, and
`meta+right`. Set `keymap = "emacs"` or
...
Suggestion navigation uses the finite `navigation-up`, `navigation-down`,
`navigation-page-up`, `navigation-page-down`, `navigation-first`, and
`navigation-last` actions. The defaults use the arrow and Page keys; first and
last are disabled until explicitly bound, so Home and End retain normal line
editing behavior. Control-key alternatives such as Ctrl+P and Ctrl+N may be
configured without changing the navigation model. Repeated navigation input in
one terminal read is applied as a single logical burst and rendered once.
