...
func loadShellHistory(file *os.File, shellName string) ([]historyOccurrence, error) {
	...
	for scanner.Scan() {
		...
		occurrence.Command = strings.TrimSpace(occurrence.Command)
		if isSafeInteractiveHistoryCommand(occurrence.Command) {
...
func isSafeInteractiveHistoryCommand(command string) bool {
	command = strings.TrimSpace(command)
	return command != "" && strings.IndexFunc(command, unicode.IsControl) == -1
}
