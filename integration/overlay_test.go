...
func TestBottomPromptGhostTextUsesChatboxSurfaceBackground(t *testing.T) {
	...
	if !strings.Contains(rendered, "48;2;36;36;40") {
	...
	if !strings.Contains(cleared, "48;2;36;36;40") {
...
func TestGhostText_MenuSync(t *testing.T) {
	...
	// Ghost text may preview the top suggestion without making it the active
	// execution target. The first navigation action explicitly selects it.
	if selected := o.GetCurrentCmd(); selected != "" {
		t.Fatalf("expected no active suggestion before navigation, got %q", selected)
	}
	if moved, selected := o.MoveCursor("down"); !moved || selected != "git checkout -b first" {
		t.Fatalf("expected down to select the first suggestion, moved=%v selected=%q", moved, selected)
	}
	if ghost := o.GetGhostText("git check", true); ghost != "out -b first" {
		t.Fatalf("expected first selected ghost text, got %q", ghost)
	}

	// A subsequent navigation action advances to item 1.
	o.MoveCursor("down")
...
func TestNormalSuggestionsStartWithoutAnExecutionSelection(t *testing.T) {
	o := NewOverlay(false)
	...
	if selected := o.GetCurrentCmd(); selected != "" {
		t.Fatalf("expected the typed command to remain the execution target, got active suggestion %q", selected)
	}
	if ghost := o.GetGhostText("git", true); ghost != " status" {
		t.Fatalf("expected top suggestion to remain available as ghost text, got %q", ghost)
	}
	rendered := o.Render()
	if strings.Contains(rendered, "0/2") {
		t.Fatalf("expected an unfocused suggestion count instead of a false zero selection, got %q", rendered)
	}
	for _, expected := range []string{"2 suggestions", "Run typed", "Select"} {
		if !strings.Contains(rendered, expected) {
			t.Fatalf("expected unfocused overlay to explain %q, got %q", expected, rendered)
		}
	}
}

func TestRecentHistoryRendersOldestToNewestAndSelectsNewestExecution(t *testing.T) {
	o := NewOverlay(false)
	o.SetRecentHistory([]RichHistoryResult{
		{HistoryEntry: HistoryEntry{ID: "new", Command: "just test", Cwd: "/repo"}, RelativeTime: "now"},
		{HistoryEntry: HistoryEntry{ID: "old", Command: "git status", Cwd: "/repo"}, RelativeTime: "1m ago"},
	})

	if !o.GetUserNavigated() {
		t.Fatal("expected empty-prompt history to have an explicit selection")
	}
	if got := o.GetCurrentCmd(); got != "just test" {
		t.Fatalf("expected newest execution to be selected, got %q", got)
	}
	if !o.RichHistory {
		t.Fatal("expected execution metadata rendering rather than ordinary suggestions")
	}
	if len(o.Items) != 2 || o.Items[0].Cmd != "git status" || o.Items[1].Cmd != "just test" {
		t.Fatalf("expected history rows oldest-to-newest, got %+v", o.Items)
	}
	if len(o.HistoryResults) != 2 || o.HistoryResults[0].ID != "old" || o.HistoryResults[1].ID != "new" {
		t.Fatalf("expected rendered metadata to stay aligned with oldest-to-newest rows, got %+v", o.HistoryResults)
	}
	if o.Cursor != 1 {
		t.Fatalf("expected newest execution selected at the bottom, got cursor %d", o.Cursor)
	}
	if moved, got := o.MoveCursor("up"); !moved || got != "git status" {
		t.Fatalf("expected Up to move visually upward to the older execution, moved=%v got=%q", moved, got)
	}
	if moved, got := o.MoveCursor("down"); !moved || got != "just test" {
		t.Fatalf("expected Down to move visually downward to the newer execution, moved=%v got=%q", moved, got)
	}
}

func TestShellFunctionSuggestionsUseDistinctFunctionBadge(t *testing.T) {
	originalProfile := lipgloss.ColorProfile()
	lipgloss.SetColorProfile(termenv.TrueColor)
	t.Cleanup(func() { lipgloss.SetColorProfile(originalProfile) })
	if got := suggestionBadgeLabel(spec.Suggestion{Source: "function"}); got != "function" {
		t.Fatalf("expected function source label, got %q", got)
	}
...
func TestLegacySuggestionDescriptionLabelsFunctions(t *testing.T) {
	...
	if !strings.Contains(rendered, "function") {
		t.Fatalf("expected legacy layouts to identify function suggestions, got %q", rendered)
	}

	classic := renderLegacySuggestionDescription(suggestion, false, 40, lipgloss.NewStyle(), theme, true)
	if !strings.Contains(classic, "function: Push the current branch") {
		t.Fatalf("expected classic layouts to identify function suggestions, got %q", classic)
	}
}

func TestSuggestionNavigationMakesTheExecutionTargetExplicit(t *testing.T) {
	o := NewOverlay(false)
	o.SetQueryAndItems("git", []spec.Suggestion{
		{Cmd: "git status"},
		{Cmd: "git switch main"},
	...
	if selected := o.CycleCursor(); selected != "git status" {
		t.Fatalf("expected first Tab to select the first suggestion, got %q", selected)
	}
	if current := o.GetCurrentCmd(); current != "git status" {
		t.Fatalf("expected selected suggestion to become the execution target, got %q", current)
	}
	rendered := o.Render()
	for _, expected := range []string{"1/2", "Run selected"} {
		if !strings.Contains(rendered, expected) {
			t.Fatalf("expected focused overlay to explain %q, got %q", expected, rendered)
		}
	}
}

func TestSuggestionRefreshPreservesAnExplicitSelectionByCommand(t *testing.T) {
	o := NewOverlay(false)
	o.SetQueryAndItems("git", []spec.Suggestion{{Cmd: "git status"}, {Cmd: "git switch main"}})
	o.MoveCursor("down")
	o.MoveCursor("down")

	o.RefreshQueryAndItems("git s", []spec.Suggestion{{Cmd: "git stash"}, {Cmd: "git switch main"}, {Cmd: "git status"}})
	if selected := o.GetCurrentCmd(); selected != "git switch main" {
		t.Fatalf("expected refresh to preserve the selected command, got %q", selected)
	}
}

func TestUpdatingIndicatorOnlyAppearsWhileEnrichmentIsPending(t *testing.T) {
	o := NewOverlay(false)
	o.SetQueryAndItems("git", []spec.Suggestion{{Cmd: "git status"}})
	o.SetUpdating(true)
	if rendered := o.Render(); !strings.Contains(rendered, "updating") {
		t.Fatalf("expected pending enrichment indicator, got %q", rendered)
	}
	o.SetUpdating(false)
	if rendered := o.Render(); strings.Contains(rendered, "updating") {
		t.Fatalf("expected completed enrichment to hide indicator, got %q", rendered)
	}
}

func TestSingleSuggestionExplainsImmediateAccept(t *testing.T) {
	o := NewOverlay(false)
	o.SetQueryAndItems("git", []spec.Suggestion{{Cmd: "git status"}})

	rendered := o.Render()
	if !strings.Contains(rendered, "Accept") || !strings.Contains(rendered, "Run typed") {
		t.Fatalf("expected the single-result footer to distinguish Tab acceptance from Enter, got %q", rendered)
	}
}

func TestUpFromUnfocusedSuggestionsSelectsTheLastResult(t *testing.T) {
	o := NewOverlay(false)
	o.SetQueryAndItems("git", []spec.Suggestion{
		{Cmd: "git status"},
		{Cmd: "git switch main"},
	...
	if moved, selected := o.MoveCursor("up"); !moved || selected != "git switch main" {
		t.Fatalf("expected up to select the last suggestion, moved=%v selected=%q", moved, selected)
	}
}

func TestArrowNavigationWrapsAcrossSuggestionListBoundaries(t *testing.T) {
	tests := []struct {
		name       string
		setup      func(*Overlay)
		firstMove  string
		...
		{
			name: "predictions",
			setup: func(o *Overlay) {
				o.SetQueryAndItems("git", []spec.Suggestion{{Cmd: "git status"}, {Cmd: "git switch main"}})
				o.FocusFirst()
			},
			firstMove: "up", firstWant: "git switch main",
			secondMove: "down", secondWant: "git status",
		},
		{
			name: "history search",
			setup: func(o *Overlay) {
				o.SetHistorySearch("deploy", "global", []spec.Suggestion{{Cmd: "deploy staging"}, {Cmd: "deploy production"}})
			},
			firstMove: "up", firstWant: "deploy production",
			secondMove: "down", secondWant: "deploy staging",
		},
		{
			name: "empty-prompt history",
			setup: func(o *Overlay) {
				o.SetRecentHistory([]RichHistoryResult{
					{HistoryEntry: HistoryEntry{Command: "newest"}},
					{HistoryEntry: HistoryEntry{Command: "oldest"}},
				})
			},
			firstMove: "down", firstWant: "oldest",
			secondMove: "up", secondWant: "newest",
		},
	}
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			if moved, selected := o.MoveCursor(test.firstMove); !moved || selected != test.firstWant {
				t.Fatalf("expected %s to wrap to %q, moved=%v selected=%q", test.firstMove, test.firstWant, moved, selected)
			}
			if moved, selected := o.MoveCursor(test.secondMove); !moved || selected != test.secondWant {
				t.Fatalf("expected %s to wrap to %q, moved=%v selected=%q", test.secondMove, test.secondWant, moved, selected)
			}
		})
	}
}

func TestNavigationSupportsPagingAndFirstLastJumps(t *testing.T) {
	o := NewOverlay(false)
	items := make([]spec.Suggestion, 8)
	for index := range items {
		items[index] = spec.Suggestion{Cmd: fmt.Sprintf("item-%d", index)}
	}
	o.SetQueryAndItems("item", items)
	o.FocusFirst()
	o.LastWindowSize = 3

	if result := o.Navigate(NavigationPageDown); !result.Moved || result.SelectedCmd != "item-3" || result.Wrapped {
		t.Fatalf("expected one page down to select item-3 without wrapping, got %+v", result)
	}
	if result := o.Navigate(NavigationPageDown); !result.Moved || result.SelectedCmd != "item-6" || result.Wrapped {
		t.Fatalf("expected a second page down to select item-6 without wrapping, got %+v", result)
	}
	if result := o.Navigate(NavigationPageDown); !result.Moved || result.SelectedCmd != "item-1" || !result.Wrapped {
		t.Fatalf("expected page down to wrap by the visible page size, got %+v", result)
	}
	if result := o.Navigate(NavigationLast); !result.Moved || result.SelectedCmd != "item-7" || result.Wrapped {
		t.Fatalf("expected last to jump directly to item-7, got %+v", result)
	}
	if result := o.Navigate(NavigationFirst); !result.Moved || result.SelectedCmd != "item-0" || result.Wrapped {
		t.Fatalf("expected first to jump directly to item-0, got %+v", result)
	}
}

func TestNavigationWrapNoticeRendersOnce(t *testing.T) {
	o := NewOverlay(false)
	o.SetQueryAndItems("git", []spec.Suggestion{{Cmd: "git status"}, {Cmd: "git switch main"}})
	o.FocusFirst()

	if result := o.Navigate(NavigationUp); !result.Wrapped {
		t.Fatalf("expected moving up from the first result to wrap, got %+v", result)
	}
	if rendered := o.Render(); !strings.Contains(rendered, "wrapped") {
		t.Fatalf("expected one understated wrap notice, got %q", rendered)
	}
	if rendered := o.Render(); strings.Contains(rendered, "wrapped") {
		t.Fatalf("expected wrap notice to clear after one render, got %q", rendered)
	}
}

func TestSuggestionRefreshPreservesStableSelectionIdentityBeforeCommandFallback(t *testing.T) {
	o := NewOverlay(false)
	o.SetQueryAndItems("deploy", []spec.Suggestion{
		{ID: "staging", Cmd: "deploy", Desc: "staging"},
		{ID: "production", Cmd: "deploy", Desc: "production"},
	})
	o.FocusFirst()
	o.MoveCursor("down")

	o.RefreshQueryAndItems("deploy", []spec.Suggestion{
		{ID: "staging", Cmd: "deploy", Desc: "updated staging"},
		{ID: "production", Cmd: "deploy", Desc: "updated production"},
	})
	if o.Cursor != 1 || o.Items[o.Cursor].ID != "production" {
		t.Fatalf("expected refresh to retain the exact selected suggestion, cursor=%d items=%+v", o.Cursor, o.Items)
	}
}

func TestHistorySearchRefreshPreservesStableSelectionIdentity(t *testing.T) {
	o := NewOverlay(false)
	o.SetHistorySearch("deploy", "global", []spec.Suggestion{
		{ID: "staging", Cmd: "deploy", Desc: "staging"},
		{ID: "production", Cmd: "deploy", Desc: "production"},
	})
	o.MoveCursor("down")
	...
	if o.Cursor != 1 || o.Items[o.Cursor].ID != "production" || !o.GetUserNavigated() {
		t.Fatalf("expected history refresh to retain the exact selected suggestion, cursor=%d items=%+v", o.Cursor, o.Items)
	}
}

func TestRichHistoryRefreshPreservesSelectedExecutionIdentity(t *testing.T) {
	o := NewOverlay(false)
	results := []RichHistoryResult{
		{HistoryEntry: HistoryEntry{ID: "first", Command: "just test"}, RelativeTime: "1m ago"},
		{HistoryEntry: HistoryEntry{ID: "second", Command: "just test"}, RelativeTime: "now"},
	}
	o.SetRichHistorySearch("test", "global", results)
	o.MoveCursor("down")
	results[1].RelativeTime = "1s ago"
	o.SetRichHistorySearch("test", "global", results)

	if o.Cursor != 1 || o.HistoryResults[o.Cursor].ID != "second" || !o.GetUserNavigated() {
		t.Fatalf("expected rich history refresh to retain the exact selected execution, cursor=%d results=%+v", o.Cursor, o.HistoryResults)
	}
}

func TestHistorySearchKeepsItsFirstResultSelected(t *testing.T) {
	o := NewOverlay(false)
	o.SetHistorySearch("deploy", "global", []spec.Suggestion{
		{Cmd: "just deploy staging"},
		{Cmd: "just deploy production"},
	})

	if selected := o.GetCurrentCmd(); selected != "just deploy staging" {
		t.Fatalf("expected history search to retain its explicit first selection, got %q", selected)
	}

	o.SetRichHistorySearch("deploy", "global", []RichHistoryResult{
		{HistoryEntry: HistoryEntry{Command: "just deploy staging"}},
		{HistoryEntry: HistoryEntry{Command: "just deploy production"}},
	})
	if selected := o.GetCurrentCmd(); selected != "just deploy staging" {
		t.Fatalf("expected rich history search to retain its explicit first selection, got %q", selected)
	}
}
...
func TestOverlayFooterUsesConfiguredKeybindings(t *testing.T) {
	...
	o := NewOverlay(false)
	o.UpdateItems([]spec.Suggestion{{Cmd: "git status"}, {Cmd: "git switch main"}})
	rendered := o.Render()
	...
	for _, expected := range []string{"Select", "Run typed"} {
		if !strings.Contains(rendered, expected) {
			t.Fatalf("expected footer to describe %q, got %q", expected, rendered)
		}
	}
}
...
func TestBottomPromptSuggestionMenuUsesFullWidthAlignedPromptMarkerAndRightBadges(t *testing.T) {
	...
	for _, color := range []string{"48;2;97;255;207", "48;2;97;238;255", "48;2;115;158;232", "48;2;253;125;243"} {
...
func TestClassicPromptSuggestionMenuPreservesLegacyDescriptionsAndMarker(t *testing.T) {
	...
	if !strings.Contains(rendered, "visited directory") || strings.Contains(rendered, "▶") ||
		...
		t.Fatalf("expected an unfocused classic overlay with border and description, got %q", rendered)
	}

	o.FocusFirst()
	rendered = o.Render()
	if !strings.Contains(rendered, "▶") {
		t.Fatalf("expected classic selection marker after explicit navigation, got %q", rendered)
	}
}
...
func TestBottomPromptOverlayRenderEmitsCompleteFrameAfterSelectionChanges(t *testing.T) {
	useTerminalSize(t, 120, 24)
	o := NewOverlay(false)
	o.SetBottomPrompt(true)
	o.SetQueryAndItems("git", []spec.Suggestion{
	...
	first := o.Render()
	for _, command := range []string{"git status", "git switch", "git show", "git stash"} {
		if !strings.Contains(first, command) {
			t.Fatalf("expected initial frame to contain %q, got %q", command, first)
		}
	}
	o.MoveCursor("down")
	o.MoveCursor("down")
	changed := o.Render()
	for _, command := range []string{"git status", "git switch", "git show", "git stash"} {
		if !strings.Contains(changed, command) {
			t.Fatalf("expected selection update to repaint %q after the compositor restores the menu region, got %q", command, changed)
		}
	}
}
