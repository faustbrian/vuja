...
func TestRecentHistoryImmediatelySelectsNewestExecution(t *testing.T) {
	o := NewOverlay(false)
	...
	if moved, got := o.MoveCursor("up"); moved || got != "" {
		t.Fatalf("expected Up at the first visible row to remain there, moved=%v got=%q", moved, got)
	}
	if moved, got := o.MoveCursor("down"); !moved || got != "git status" {
		t.Fatalf("expected Down to select the next visible history row, moved=%v got=%q", moved, got)
	}
}
