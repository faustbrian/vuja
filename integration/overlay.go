...
const (
	...
	noOverlaySelection       = -1
)

type NavigationAction string

const (
	NavigationUp       NavigationAction = "up"
	NavigationDown     NavigationAction = "down"
	NavigationPageUp   NavigationAction = "page-up"
	NavigationPageDown NavigationAction = "page-down"
	NavigationFirst    NavigationAction = "first"
	NavigationLast     NavigationAction = "last"
)
...
func (o *Overlay) InvalidateGeometry() {
	...
	defer o.mu.Unlock()
	o.LastWindowSize = 0
	if width, height, err := terminalSize(int(os.Stdout.Fd())); err == nil {
		o.TerminalWidth, o.TerminalHeight = width, height
	}
}

func (o *Overlay) SetTerminalSize(width, height int) {
	o.mu.Lock()
	defer o.mu.Unlock()
	if width > 0 {
...
func NewOverlay(isDarkBackground bool) *Overlay {
	...
	o := &Overlay{
		...
		scrollStyle:     lipgloss.NewStyle().Foreground(theme.ScrollInfo),
		mutedStyle:      lipgloss.NewStyle().Foreground(theme.Muted),
		footerKeyStyle:  lipgloss.NewStyle().Foreground(theme.ScrollInfo).Bold(true),
		footerTextStyle: lipgloss.NewStyle().Foreground(theme.ScrollInfo),
		Visible:         false,
		Cursor:          noOverlaySelection,
		StartIdx:        0,
...
func (o *Overlay) UpdateItems(items []spec.Suggestion) {
	...
	o.ContextLabel = ""
	o.RichHistory = false
	o.RecentHistory = false
	o.HistoryResults = nil
	o.UserNavigated = false
	o.Visible = len(o.Items) > 0
	o.Cursor = noOverlaySelection
	o.StartIdx = 0
	o.NavigationWrapped = false
}
...
func (o *Overlay) SetUserNavigated(v bool) {
	...
	defer o.mu.Unlock()
	o.UserNavigated = v
	if o.ContextLabel == "" && !o.RichHistory {
		if v && o.Cursor == noOverlaySelection && len(o.Items) > 0 {
			o.Cursor = 0
		} else if !v {
			o.Cursor = noOverlaySelection
		}
...
func (o *Overlay) RefreshQueryAndItems(query string, items []spec.Suggestion) {
	o.mu.Lock()
	defer o.mu.Unlock()
	selectedID := ""
	selectedCommand := ""
	if o.UserNavigated && o.Cursor >= 0 && o.Cursor < len(o.Items) {
		selectedID = o.Items[o.Cursor].ID
		selectedCommand = o.Items[o.Cursor].Cmd
	}
	o.TypedQuery = query
	o.ContextLabel = ""
	o.RichHistory = false
	...
	o.UserNavigated = false
	o.NavigationWrapped = false
	if index := selectedSuggestionIndex(items, selectedID, selectedCommand); index >= 0 {
		o.Cursor = index
		o.UserNavigated = true
	}
}

func (o *Overlay) SetUpdating(updating bool) {
	o.mu.Lock()
	o.Updating = updating
	o.mu.Unlock()
}

func (o *Overlay) IsUpdating() bool {
	o.mu.Lock()
	defer o.mu.Unlock()
	return o.Updating
}
...
func (o *Overlay) SetHistorySearch(query string, label string, items []spec.Suggestion) {
	o.mu.Lock()
	defer o.mu.Unlock()
	contextLabel := historyContextLabel(query, label)
	selectedID := ""
	selectedCommand := ""
	preserveSelection := o.UserNavigated && !o.RichHistory && o.ContextLabel == contextLabel &&
		o.TypedQuery == query && o.Cursor >= 0 && o.Cursor < len(o.Items)
	if preserveSelection {
		selectedID = o.Items[o.Cursor].ID
		selectedCommand = o.Items[o.Cursor].Cmd
	}
	o.TypedQuery = query
	o.ContextLabel = contextLabel
	o.UserNavigated = false
	o.Items = items
	o.RichHistory = false
	o.RecentHistory = false
...
func (o *Overlay) SetRichHistorySearch(query string, label string, results []RichHistoryResult) {
	...
	selectedID := ""
	selectedCommand := ""
	preserveSelection := o.UserNavigated && o.RichHistory && !o.RecentHistory &&
		o.TypedQuery == query && o.ContextLabel == contextLabel && o.Cursor >= 0 && o.Cursor < len(o.Items)
	if preserveSelection {
		selectedCommand = o.Items[o.Cursor].Cmd
		if o.Cursor < len(o.HistoryResults) {
			selectedID = o.HistoryResults[o.Cursor].ID
		}
	}
	o.TypedQuery = query
	o.ContextLabel = contextLabel
	...
	if preserveSelection {
		if index := selectedHistoryResultIndex(results, selectedID, selectedCommand); index >= 0 {
			o.Cursor = index
			o.UserNavigated = true
		}
	}
...
func (o *Overlay) SetRecentHistory(results []RichHistoryResult) {
	o.mu.Lock()
	defer o.mu.Unlock()
	o.TypedQuery = ""
	o.ContextLabel = "history"
	o.UserNavigated = len(results) > 0
	o.RichHistory = true
	o.RecentHistory = true
	o.HistoryResults = make([]RichHistoryResult, 0, len(results))
	o.Items = make([]spec.Suggestion, 0, len(results))
	for index := len(results) - 1; index >= 0; index-- {
		result := results[index]
		o.HistoryResults = append(o.HistoryResults, result)
		o.Items = append(o.Items, spec.Suggestion{
			ID: result.ID, Cmd: result.Command, Desc: "history", Icon: "history", Source: "history", Confidence: 80,
		})
	}
	o.Visible = len(results) > 0
	o.Cursor = noOverlaySelection
	if len(results) > 0 {
		o.Cursor = len(results) - 1
	}
	o.StartIdx = 0
	o.NavigationWrapped = false
}

func historyContextLabel(query, label string) string {
	contextLabel := strings.TrimSpace(label)
	if query != "" {
		contextLabel += ": " + query
	}
	return contextLabel
...
func selectedSuggestionIndex(items []spec.Suggestion, selectedID, selectedCommand string) int {
	if selectedID != "" {
	...
	if selectedCommand != "" {
		for index := range items {
			if items[index].Cmd == selectedCommand {
				return index
			}
		}
	}
...
func selectedHistoryResultIndex(results []RichHistoryResult, selectedID, selectedCommand string) int {
	if selectedID != "" {
		for index := range results {
			if results[index].ID == selectedID {
				return index
			}
		}
	}
	if selectedCommand != "" {
		for index := range results {
			if results[index].Command == selectedCommand {
				return index
			}
		}
	}
...
func (o *Overlay) MoveCursor(dir string) (moved bool, selectedCmd string) {
	result := o.Navigate(NavigationAction(dir))
	return result.Moved, result.SelectedCmd
}

func (o *Overlay) Navigate(action NavigationAction) NavigationResult {
	return o.NavigateMany([]NavigationAction{action})
}

func (o *Overlay) NavigateMany(actions []NavigationAction) NavigationResult {
	o.mu.Lock()
	defer o.mu.Unlock()
	if !o.Visible || len(o.Items) == 0 {
	...
	for _, action := range actions {
		step := o.navigateLocked(action)
		result.Moved = result.Moved || step.Moved
		result.Wrapped = result.Wrapped || step.Wrapped
	}
	o.NavigationWrapped = result.Wrapped
	if result.Moved && o.Cursor >= 0 && o.Cursor < len(o.Items) {
		result.SelectedCmd = o.Items[o.Cursor].Cmd
	}
	return result
}

func (o *Overlay) navigateLocked(action NavigationAction) NavigationResult {
	count := len(o.Items)
	if count == 0 {
		return NavigationResult{}
	}
	oldCursor := o.Cursor
	if oldCursor == noOverlaySelection {
		switch action {
		case NavigationUp, NavigationPageUp, NavigationLast:
			o.Cursor = count - 1
		case NavigationDown, NavigationPageDown, NavigationFirst:
			o.Cursor = 0
		default:
			return NavigationResult{}
		}
		return NavigationResult{Moved: true, SelectedCmd: o.Items[o.Cursor].Cmd}
	}
	...
	case NavigationPageUp:
		pageSize := o.navigationPageSizeLocked()
		raw := o.Cursor - pageSize
		wrapped = raw < 0
		o.Cursor = positiveModulo(raw, count)
	...
	if o.Cursor == oldCursor {
		return NavigationResult{Wrapped: wrapped}
	}
	return NavigationResult{Moved: true, SelectedCmd: o.Items[o.Cursor].Cmd, Wrapped: wrapped}
}

func (o *Overlay) navigationPageSizeLocked() int {
	if o.LastWindowSize > 0 {
		return min(o.LastWindowSize, len(o.Items))
	}
	height := o.TerminalHeight
	if height <= 0 {
		height = 24
	}
	if o.BottomPrompt {
		height -= max(o.PromptRows, 1)
	}
	return min(visibleItemLimit(config.Get().UI.MaxHeight, height), len(o.Items))
}
...
func (o *Overlay) draw() string {
	...
	navigationWrapped := o.NavigationWrapped
	o.NavigationWrapped = false

	t := o.theme
	border := o.borderStyle
	scrollStyle := o.scrollStyle
	...
	width, height := o.TerminalWidth, o.TerminalHeight
	if width <= 0 {
		width = 120
	}
	if height <= 0 {
	...
	emitRow := func(row int, content string) {
		moveToRow(row)
	...
	if len(o.Items) > 0 {
		counter := "1 suggestion"
		if len(o.Items) > 1 {
			counter = fmt.Sprintf("%d suggestions", len(o.Items))
		}
		if o.Cursor >= 0 {
	...
	if bottomPrompt {
		...
		if scrollInfo != "" {
			row.WriteString(strings.Repeat(" ", overlayHorizontalPadding))
			row.WriteString(scrollStyle.Render(strings.TrimSpace(scrollInfo)))
		}
		emitRow(0, row.String())
	} else {
		var row strings.Builder
		fmt.Fprintf(&row, "%s%s%s%s%s",
			border.Render("╭"),
	...
	for i := start; i < end; i++ {
		...
		if emptyRichHistory {
			...
			fmt.Fprintf(&row, "%s%s%s%s",
				left,
				o.mutedStyle.Render(strings.Repeat(" ", leadingPadding)+message),
				o.mutedStyle.Render(strings.Repeat(" ", trailingPadding)),
				right,
			)
			emitRow((i-start)+1, row.String())
			continue
		}
		...
		if o.RichHistory && i < len(o.HistoryResults) {
			...
			historyConfig := config.Get().UI.History
			if o.RecentHistory {
				historyConfig.ShowCwd = true
				historyConfig.ShowExitStatus = true
			}
			historyRow := renderRichHistoryRow(o.HistoryResults[i], selected, rowWidth, t, historyConfig)
			fmt.Fprintf(&row, "%s%s%s%s%s%s%s",
				left,
		...
		fmt.Fprintf(&row, "%s%s%s%s%s%s%s%s%s%s",
			left,
			bg.Render(strings.Repeat(" ", leadingPadding)),
			markerStyle.Render(marker),
			bg.Render(" "),
	...
	footerInfo := ""
	if !isClassic {
		keyStyle := o.footerKeyStyle
		textStyle := o.footerTextStyle
		var hints []string
		runTarget := " Run typed"
		if o.Cursor >= 0 {
			runTarget = " Run selected"
		}
		hints = append(hints, keyStyle.Render("<Enter>")+textStyle.Render(runTarget))
		if label := keybindingLabel(config.Get().Keybindings.Accept); label != "" {
			...
			hints = append(hints, keyStyle.Render(label)+textStyle.Render(action))
		}
	...
	if bottomPrompt {
		...
		if footerInfo != "" {
			...
			row.WriteString(strings.Repeat(" ", max(currentBoxWidth-footerWidth-trailingPadding, 0)))
			row.WriteString(footerInfo)
		}
		emitRow(windowSize+1, row.String())
	} else {
		var row strings.Builder
		fmt.Fprintf(&row, "%s%s%s%s%s",
			border.Render("╰"),
...
func renderLegacySuggestionDescription(
	...
	if classic {
		...
		if suggestion.Source == "function" {
			text = "function: " + text
			descriptionColor = theme.ScrollInfo
		} else if suggestion.Icon == "alias" {
			text = "alias: " + text
		}
	...
	color := theme.Muted
	switch {
	case suggestion.Source == "function":
		label, color = "function", theme.ScrollInfo
	case suggestion.Icon == "alias":
		label, color = "alias", theme.ScrollInfo
	case suggestion.Icon == "history":
		label, color = "history", theme.Accent
	case suggestion.Icon == "system":
