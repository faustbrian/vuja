...
func TestSearchCachedHistoryDoesNotWaitForTheHistoryLoader(t *testing.T) {
	mu.Lock()
	defer mu.Unlock()

	if results, available := SearchCachedHistory("git", nil); available || len(results) != 0 {
		t.Fatalf("expected a cache miss while history is busy, available=%v results=%v", available, results)
	}
}
