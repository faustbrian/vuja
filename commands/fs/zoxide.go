...
func ZoxideGenerator() spec.GeneratorFunc {
	return func(ctx context.Context, tokens []string, prefix string, partial string) []spec.Suggestion {
		fullQuery := strings.Join(tokens[1:], " ")
		localSuggestions := spec.FileGenerator("/")(ctx, tokens, prefix, fullQuery)
