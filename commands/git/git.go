...
func GitRemoteGenerator(ctx context.Context, tokens []string, _ string, _ string) []spec.Suggestion {
	return getGitResults(ctx, tokens, "remote")
}

func GitStashGenerator(ctx context.Context, tokens []string, _ string, _ string) []spec.Suggestion {
	return getGitResults(ctx, tokens, "stash", "list", "--format=%gd: %gs")
}

func GitTagGenerator(ctx context.Context, tokens []string, _ string, _ string) []spec.Suggestion {
	return getGitResults(ctx, tokens, "tag", "-l")
}

func GitCommitGenerator(ctx context.Context, tokens []string, _ string, _ string) []spec.Suggestion {
	cwd := spec.GetCWD()
...
func getGitResults(ctx context.Context, tokens []string, args ...string) []spec.Suggestion {
	return getGitResultsFiltered(ctx, tokens, false, args...)
}

func getGitResultsFiltered(ctx context.Context, tokens []string, localOnly bool, args ...string) []spec.Suggestion {
	cwd := spec.GetCWD()
...
func GitBranchGenerator(ctx context.Context, tokens []string, _ string, _ string) []spec.Suggestion {
	...
	return getGitResults(ctx, tokens, "branch", "-a", "--format=%(refname:short)")
}
...
func gitLocalBranchGenerator(ctx context.Context, tokens []string, _ string, _ string) []spec.Suggestion {
	isCreateMode := false
	for _, t := range tokens {
		if t == "-b" || t == "-B" || t == "-c" || t == "-C" {
			isCreateMode = true
	...
	return getGitResultsFiltered(ctx, tokens, true, "branch", "-a", "--format=%(refname:short)")
}

func GitPushPullGenerator(ctx context.Context, tokens []string, prefix string, partial string) []spec.Suggestion {
	...
	if len(pArgs) == 0 {
		return GitRemoteGenerator(ctx, tokens, prefix, partial)
	}
	...
	return gitLocalBranchGenerator(ctx, tokens, prefix, partial)
}
		...
		Subcommands: []spec.Subcommand{
			...
			},
			{
				...
				Generator: func(ctx context.Context, tokens []string, prefix string, partial string) []spec.Suggestion {
					...
					branches := GitBranchGenerator(ctx, tokens, prefix, partial)
					files := spec.FileGenerator()(ctx, tokens, prefix, partial)
			...
			},
			{
				...
				Generator: func(ctx context.Context, tokens []string, prefix string, partial string) []spec.Suggestion {
					tags := GitTagGenerator(ctx, tokens, prefix, partial)
					commits := GitCommitGenerator(ctx, tokens, prefix, partial)
